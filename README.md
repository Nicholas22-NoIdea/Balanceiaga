...
- **AI Task Analyzer** — students paste a task description and the app breaks it into subtasks with hour estimates, mental demand, urgency, and flexibility — built as a fast, rule-based simulation rather than a live LLM call (see note below)
...
| B — AI-assisted PDF/syllabus parser <br>(Chosen)                 | Kept as the "wow factor" feature — this is a good use of AI (structured JSON extraction) rather than using AI to make scheduling decisions, so it doesn't hit the same "black-box" concern. |
| C — Full AI-driven scheduling <br>(Dropped)                      | Both early concepts (from Claude and Gemini's brainstorms) leaned on AI to detect urgency/flexibility and make scheduling calls directly. Dropped after mentor feedback — no dataset to train or validate this, and it would be hard to explain *why* the AI made a given decision. |
...
| E — Rigid, locked daily time slots <br>(Dropped)                 | Original design locked tasks into fixed slots once scheduled. Mentor feedback (see 2.3) flagged this as something that could increase anxiety, not reduce it. Replaced with flexible micro-milestones the student can adjust. |
...
A structured diagram illustrating the root causes, core issue, and downstream academic/physical effects of student burnout. image
image
...
**UI Prototype:** [ [https://ivanmaktech-balanceiaga.vercel.app/](https://ivanmaktech-balanceiaga.vercel.app/)  ]
...
imageUser can instantly see how heavy their day is 
Load Breakdown allow user to see what’s consuming their time and energy ; Capacity View allow user know how much time they realistically have ; Monthly Calendar allow user yo plan ahead and prevent overload
imageMonthly calendar allow user see workload at a glance across the month ; Google Calender bring your existing commitments here

imageDaily Schedule allow user to edit, move and organise task around your day
imageShow user there are overloaded
imageAl rebalance option where it analyse deadline, duration, priority, flexibility, consequence, protected time or user can edit to fine-tune their plan manually
imageRecovery Recommendations where user can get activities matched to their current needs
imageSchedule Recovery to fit user avaibility
